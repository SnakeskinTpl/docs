...
Одним из способов декларации переопределяемых структур являются блоки ([вызываемые блоки](#{@@api}#block) являются частным случаем),
...
Если нам нужно доопределить родительский блок, то для этого следует использовать директиву [super](#{@@api}#super).
