...
Snakeskin удобно использовать для генерации статичных страниц -- транслятор может скопмилировать и сразу выполнить шаблон, возвратив результат его работы, что удобно использовать для генерации шаблонов для различных MVVM-фреймворков, таких как Angular, Vue или React. В принципе, можно использовать Snakeskin даже для генерации кода на PHP - но помните, мы не гарантируем защиты от динозавров, которые могут прийти за вами.

Snakeskin поддерживает интеграцию с популярными task-runnerами и сборщиками, такими как Grunt, Gulp или Webpack с помощью плагинов -- это позволяет бесшовно встроить его в систему сборки любого проекта.
