- namespace docs.ru.api['Директивы']['1::Создание шаблона']['2::template']
...
Директива **template** декларирует шаблон c заданным именем и входными параметрами.
...
можно поменять задав специальный [renderMode](#api--renderMode) или явно вернув значение через директиву [return](#return).
...
шаблон не может включать в себя другой шаблон - для этого есть подшаблоны.
...
реализует [стандартный механизм декларации параметров](guide.html#introParams).
...
`TPL_NAME` - строка, которая содержит полное имя шаблона вместе с простраством имён, причём именно в том виде,
как оно было задано при декларации шаблона;

`PARENT_TPL_NAME` - строка, которая содержит полное имя родительского шаблона вместе с простраством имён,
...
При декларации шаблона ему можно задать определённые [параметры трансляции](guide.html#introSet), для этого используется
...
[Подробнее про наследование](guide.html#inheritBasic).
