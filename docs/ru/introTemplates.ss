...
Если в JavaScript результатом работы функции является то, что указано после оператора *return*, то в Snakeskin любой
текст автоматически является результатом, хотя возможность явного возврата значения также имеется ([return](api.html#return)).
...
Snakeskin позволяет разработчику вставлять в статичный текст шаблона значения JavaScript выражений, например,
вызовы функций, значения переменных, результат математических операций и т.д.. Для этого используется специальная директива
[output](api.html#output). Давайте усложним пример выше и будем выводить приветствие по заданному параметру-имени.
