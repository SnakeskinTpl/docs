...
[localization](#{@@api}#compile--localization) (включён по умолчанию) будут автоматически оборачиваться функцией `i18n()`.
...
Имя используемой функции локализации можно задать явно с помощью параметра [i18nFn](#{@@api}#compile--i18nFn).
...
[i18nFnOptions](#{@@api}#compile--i18nFnOptions), например:
...
Или же можно просто отключить параметр [localization](#{@@api}#compile--localization).
...
например, можно было передать его переводчикам. За эту функцию отвечает параметр [words](#{@@api}#compile--words).
