...
Параметр задаёт название [фильтра](guide#{@@prfx}.html#filters) для обработки [липких ссылок](#tag--Ссылки_на_родительский_класс),
...
Объект [фильтров](guide#{@@prfx}.html#filters) по умолчанию для директивы [output](#output):
...
Если параметр равен `false`, то [строки локализации](guide#{@@prfx}.html#localization) будут отключены, т.е. сохраняются "как есть".
...
Название используемой глобальной функции локализации: данная функция будет оборачивать [строки локализации](guide#{@@prfx}.html#localization).
...
Если задан данный параметр, то [строки локализации](guide#{@@prfx}.html#localization) будут заменятся на этапе трансляции.
...
Если задан данный параметр, то найденные в шаблонах [строки локализации](guide#{@@prfx}.html#localization) будут добавляться
