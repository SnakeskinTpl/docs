...
Директива **template** декларирует шаблон c заданным именем и выходными параметрами.
...
Начать декларацию шаблонов можно только после декларации пространства имён, причём в рамках одного пространства имён
не может быть 2-х шаблонов с одинаковым именем, а размещаться шаблон может только в глобальной области декларации, т.е.
шаблон не может включать в себя другой шаблон.
...
реализует [стандартный механизм декларации](guide.html#introParams).
...
Шаблон Snakeskin поддерживает специальные модификаторы декларации.
