- namespace docs.en.guide['2::Basics']['3::Декларация шаблона и вывод данных']
...
Код, который находится вне тела шаблона or прототипа считается глобальным и помимо трансляции в JavaScript
...
[output](#{@@api}#output) or [call](#{@@api}#call), а остальные директивы считаются логическими, например,
[if](#{@@api}#if) or [for](#{@@api}#for).

Logic directives не участвуют в обработке пробелов, т.е.:
