- namespace docs.ru.api['Директивы']['3::Глобальный контекст']['1::eval']
...
Директива **eval** создаёт блок, который выполнится на этапе трансляции, но не войдёт в конечный JS.
...
то директива *eval* позволяет создать блок, содержимое которого будет исключаться из результируещего кода, но по прежнему
