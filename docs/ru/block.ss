- namespace docs.ru.api['Директивы']['1::Создание шаблона']['5::block']
...
Директива **block** декларирует статичный блок или подшаблон c заданным именем и входными параметрами.
...
специальный [renderMode](#api--renderMode) или явно вернув значение через директиву [return](#return).
...
и вызываемые лежает в одном пространстве имён. В рамках шаблона не может быть 2-х блоков с одинаковым названием.
...
явно указывать к какому шаблону он принадлежит, например:
...
котого он используется - это удобно для организации рекурсий, например:
