- namespace docs.en.guide['2::Основы']['3::Декларация шаблона и вывод данных']
...
Объявление шаблона возможно с помощью директив [template](api#{@@prfx}.html#template), [interface](api#{@@prfx}.html#interface) и
[placeholder](api#{@@prfx}.html#placeholder). Шаблон не может включать в себя другой шаблон, для этого есть другие директивы.
...
[namespace](api#{@@prfx}.html#namespace), причём в рамках одного файла Snakeskin может существовать только одно пространство имён,
т.е.:
...
[output](api#{@@prfx}.html#output). Давайте усложним пример данный выше: будем выводить приветствие по заданному параметру-имени.
...
Узнать о всех директивах, которые поддерживает Snakeskin, можно в [документации проекта](api#{@@prfx}.html).
...
Если не задан параметр [tolerateWhitespaces](api#{@@prfx}.html#compile--tolerateWhitespaces), то любые пробельные символы
...
Исключение составляют блоки [cdata](#api#{@@prfx}.html#cdata), литералы строк и регулярных выражений внутри директивы и
[jsDoc комментарии](#basics--jsDoc).

С помощью параметра [ignore](api#{@@prfx}.html#compile--ignore) можно задать те пробельные символы, которые будут полностью
...
[output](api#{@@prfx}.html#output) или [call](api#{@@prfx}.html#call), а остальные директивы считаются логическими, например,
[if](api#{@@prfx}.html#if) или [for](api#{@@prfx}.html#for).
