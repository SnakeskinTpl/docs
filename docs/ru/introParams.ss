...
С помощью оператора `?` можно задать, чтобы значение по умолчанию ставилось только при значении `undefined`.
...
Существует также оператор `!`, которые отменяет такое поведение, но он используется только при наследовании блоков и шаблонов.
