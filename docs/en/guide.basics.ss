- namespace docs.en.guide['2::Basics']['1::Декларация и базовый синтаксис']
...
то используется специальный расширенный синтаксис декларации директив (символы `\#{` и `}`), например:
...
Ряд директив Snakeskin поддерживают специальный синтаксис интерполяции значений, который нужен для проброса динамических
выражений в статический текст через конструкцию `${выражение}`.
...
Подобно тегам HTML or XML, многие директивы Snakeskin могут включать в себя другие директивы и т.д. и в дальнейшем
...
Завершающая часть, как правило, создаётся с помощью специальной директивы [end](#{@@api}#end),
