- namespace docs.en.guide['2::Basics']['4::Параметры функциональных директив']
...
У параметров может быть значение по умолчанию, которое применяется, если значение параметра будет `null` or `undefined`.
...
Существует также оператор `!`, который отменяет оператор `?`, но он используется только при наследовании блоков or шаблонов.

## with биндинг

Оператор `@` осуществляет привязку параметра к директиве [with](#{@@api}#with), например:
