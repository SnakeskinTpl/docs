- namespace docs.en.api['1::Directives']['1::Creating templates']['5::block']
...
Директива *block* несёт двойную функциональность: с одной стороны она позволяет создавать [статичные блоки](##{@@guide}#inheritBasic),
...
а т.к. директива *block* является функциональной, то она реализует [стандартный механизм декларации параметров](#{@@guide}#introParams).
По умолчанию вызываемые блоки возвращают строки, однако это поведение можно поменять задав
специальный [renderMode](#compile--renderMode) or явно вернув значение через директиву [return](#return).
...
механика наследования практически идентична со статичными блоками и шаблонами, и описана в [отдельной главе](#{@@guide}#inheritBlocks).
...
Любые блоки могут декларироваться как внутри шаблона or другого блока, так и в глобальной области, но при такой декларации
